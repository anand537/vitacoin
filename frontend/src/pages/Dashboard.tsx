...
import { smartApi } from '../lib/smartApi'
...
const Dashboard: React.FC = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const users = await smartApi.users.getAll()
      if (users.length > 0) {
  ...
  const fetchTransactions = async () => {
    try {
      if (user) {
        const transactions = await smartApi.coin.getTransactions(user.userId, 1, 5)
  ...
  const handleDailyClaim = async () => {
    try {
      ...
      const result = await smartApi.coin.addCoins({
        userId: user.userId,
        amount: 50,
